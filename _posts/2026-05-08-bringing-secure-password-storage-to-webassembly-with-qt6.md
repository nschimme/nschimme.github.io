...
5.  **Credential Entry:** 
    *   A transient HTML modal will appear. This is the **Bridge UI**.
    *   Enter a password and click **Save**.
6.  **Native Storage:** Observe the browser's native "Save Password" prompt (Chrome/Firefox/Safari/Brave). Confirm the save to store the secret in your browser's secure vault.
...
The browser is a hostile environment by design. Applications cannot simply reach out and touch the local file system or call OS-level encryption APIs. Modern browsers require a "user gesture" (like a click) to access the **Credential Management API**, which creates a significant hurdle for C++ libraries designed to run silently in the background.
...
To make this work, I implemented a bridge between the Qt/C++ world and the browser's capabilities. Since WebAssembly operates within the JavaScript context of the page, we leverage **Emscripten’s ASYNCIFY** and a transient HTML bridge.
...
2.  **The Bridge:** It injects a temporary DOM element to satisfy the "user gesture" requirement, allowing the browser's `navigator.credentials` API to fire.
3.  **The User Experience:** The browser handles the sensitive data. For the user, it "just works"—credentials can be persisted across sessions without the developer needing to roll their own (often insecure) encryption logic.
4.  **The Retrieval:** When the app restarts, `ReadPasswordJob` queries the browser's secure vault, and the password is fed back into your C++ logic seamlessly.

### Why This Matters for Developers

If you are building a cross-platform app today, security shouldn't be an afterthought. By integrating this into QtKeychain, we’ve made secure storage a "just works" feature. 
